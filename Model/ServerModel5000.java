...
public class ServerModel5000{
	...
	static boolean server3000Connected;
...
/*
	static class ThreadedServer implements Runnable {
		...
		public String getMemberOtherServer() {
			String s="";
			if(server3000Connected) {
				try {
					toOtherServer.sendMessage("GetServerList");
					s=toOtherServer.readMessage();
					System.out.println("S5000 116 : "+s);
					s=toOtherServer.readMessage();
					System.out.println("S5000 118 : "+s);
			...
			try {
				while(true) {
					...
					if(firstConnection) {
						...
						if(msgIn.equalsIgnoreCase("GetServerList")) {
		...
		public String getMemberOtherServer() {
			...
			if(server3000Connected) {
				try {
					...
					s=toOtherServer.readMessage();
					System.out.println("S5000 113 : "+s);
					s=toOtherServer.readMessage();
					System.out.println("S5000 115 : "+s);
